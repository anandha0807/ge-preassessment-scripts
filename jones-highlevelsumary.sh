...
Declare @accountids nvarchar(max) = 1146
declare @temp_accountid table(
accountid int)
insert into @temp_accountid
select * from udf_ListToTable(@accountids,',')
...
acc.AccountID in(select * from @temp_accountid)
...
a.AccountID in(select * from @temp_accountid)
and j.DeletedOn is null
...
where --a.AccountID = @AccountID  
a.AccountID in(select * from @temp_accountid)
AND u.DeletedOn is NULL and u.IsTalent=1
...
where --acc.AccountID = @AccountID 
acc.AccountID in(select * from @temp_accountid)
and j.DeletedOn is null and jf.DeletedOn is null
...
and a.JobID in (select j.JobID from Job j where j.DeletedOn is NULL and j.OwnerAccountID in (select * from @temp_accountid)))
...
where --acc.AccountID = @AccountID 
acc.AccountID in(select * from @temp_accountid)
AND a.DeletetedOn is NULL AND j.DeletedOn is null
...
where --a.AccountID = @AccountID 
a.AccountID in(select * from @temp_accountid)
AND j.DeletedOn is null and aa.DeletetedOn is null
...
where --acc.AccountID = @AccountID  
acc.AccountID in(select * from @temp_accountid)
and acc.DeletedOn is null and l.DeletedOn is null
...
where --acc.AccountID = @AccountID 
acc.AccountID in(select * from @temp_accountid)
and acc.DeletedOn is null and u.DeletedOn is null and lg.Name is not null
...
acc.AccountID in(select * from @temp_accountid)
and acc.DeletedOn is null AND l.DeletedOn is null
--AND gu.Guest= 0 --Excluded shared Lightbox to Guest users (2652)
...
inner join AssetHistoryItemType hit on hit.AssetHistoryItemTypeID = ah.AssetHistoryItemTypeID
inner join ApprovalGallery agl on ah.ApprovalGalleryID=agl.ApprovalGalleryID
 left join ApprovalGalleryCollection agc on agl.ApprovalGalleryID=agc.ApprovalGalleryID 
where --a.AccountID = @AccountID 
a.AccountID in(select * from @temp_accountid)
and ah.ApprovalGalleryID is not null
and a.DeletedOn is null and a.DeletedBy is null and agl.DeleteDate is null and agl.DeletedBy is null
...
where --a.AccountID = @AccountID
a.AccountID in(select * from @temp_accountid)
 and a.DeletedOn is null and a.DeletedBy is null and ag.DeleteDate is null and ag.DeletedBy is null
...
where --a.AccountID = @AccountID 
a.AccountID in(select * from @temp_accountid)
and c.NotesHistoryID is not null 
...
and a.AccountID in(select * from @temp_accountid)
and j.DeletedOn is null
and jf.DeletedOn is null
AND jf.JobFolderID is NOT NULL
...
[watermarks id-mappings] bigint, [PROJECTASSIGNMENTS ID-MAPPINGS] bigint
)
                 ...
                 FOR HighLevelSummary IN (Accounts_total_records,
                                 ...
                                 [approval-gallery-history id-mappings],
                                                                 ...
                                                                 [assets watermarked-incomplete-by-account],[user-scope-policies id-mappings],[assets watermarked-incomplete],[metadata stage info],[assets info],
                                                                                                                                 ...
                                                                                                                                 [PROJECTASSIGNMENTS ID-MAPPINGS]
                                                                 )
                 ) AS UnPivotExample
UNION 
...
update @output_table set SortOrder=23 where HighLevelSummary='PROJECTASSIGNMENTS ID-MAPPINGS'
...
select HighLevelSummary, CASE Globaledit_Legacy WHEN -9999 THEN 0 ELSE Globaledit_Legacy END AS Globaledit_Legacy From @output_table where SortOrder is not null order by SortOrder" -s , -W -k1 > Output/"$name"_HighLevel_Summary.csv

sed -e 's/-,//g;s/-//g;s/,,//g;/^$/d' Output/"$name"_HighLevel_Summary.csv > Final_CSV/"$name"_HighLevel_Summary.csv
grep -v "^--" Final_CSV/"$name"_HighLevel_Summary.csv > Final_CSV/"$name"_HighLevel_Summary.csv
